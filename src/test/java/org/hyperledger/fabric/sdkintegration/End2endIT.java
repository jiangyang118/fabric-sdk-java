...
public class End2endIT {
	...
	public void runFabricTest(final SampleStore sampleStore) throws Exception {
		...
		runChannel(client, barChannel, true, sampleOrg, 100); // run a newly constructed bar channel with different b
																// value!
	...
	public void enrollUsersSetup(SampleStore sampleStore) throws Exception {
		...
		for (SampleOrg sampleOrg : testSampleOrgs) {
			...
			System.err.println("sampleOrgName = " + sampleOrgName + "sampleOrgDomainName = " + sampleOrgDomainName);
			System.err.println(testConfig.getTestChannelPath());
			System.err.println(format("/users/Admin@%s/msp/keystore", sampleOrgDomainName));
			File f = Util
					.findFileSk(Paths
			...
			File f2 = Paths.get(testConfig.getTestChannelPath(), "crypto-config/peerOrganizations/",
					sampleOrgDomainName,
	...
	void runChannel(HFClient client, Channel channel, boolean installChaincode, SampleOrg sampleOrg, int delta) {
		...
		try {
			 ...
			 * PLEASE READ !!
			 *
			 * The following is using Fabric v1.0 APIs for testing and demoing backward
			 * compatibility. After v2.0 Fabric release unless there is a need for this in
			 * your application it is highly encouraged to move to Fabric v2.0 capabilities
			 * and use the new v2.0 Lifecycle APIs for managing chaincode.
			 * 
			 * @see <a href=
			 * "https://github.com/hyperledger/fabric-sdk-java/blob/master/docs/release_v2.0.0_notes.md#fabj-288-lifecycle-chaincode-management"
			 * </a>
			 *
			 */
			/////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if (installChaincode) {
				...
				if (isFooChain) {
					...
					if (testConfig.isFabricVersionAtOrAfter("1.1")) { // Fabric 1.1 added support for META-INF in the
						...
						// The file IndexA.json as part of the META-INF will be packaged with the source
						// to create the index.
						installProposalRequest
								.setChaincodeMetaInfLocation(new File("src/test/fixture/meta-infs/end2endit"));
					}
				} else {
					...
					if (CHAIN_CODE_LANG.equals(Type.GO_LANG)) {
						...
						installProposalRequest.setChaincodeInputStream(Util.generateTarGzInputStream(
								CHAIN_CODE_FILEPATH.resolve(chaincodeSrcPath).toFile(), chaincodeSrcPath.toString()));
					} else {
						installProposalRequest.setChaincodeInputStream(
								Util.generateTarGzInputStream(CHAIN_CODE_FILEPATH.toFile(), "src"));
					}
				}
...
//        boolean doPeerEventing = !testConfig.isRunningAgainstFabric10() && FOO_CHANNEL_NAME.equals(name);
		...
		System.err.println(configTxPath);
		System.err.println(configTxFile);
				...
				for (BlockInfo.EnvelopeInfo envelopeInfo : returnedBlock.getEnvelopeInfos()) {
					...
					if (envelopeInfo.getType() == TRANSACTION_ENVELOPE) {
						...
						for (BlockInfo.TransactionEnvelopeInfo.TransactionActionInfo transactionActionInfo : transactionEnvelopeInfo
							...
							long excpectedStatus = current == 2 && i == 1 && j == 1
									? expectedMoveRCMap.get(channel.getName())
									: 200; // only transaction we changed the status code.
							assertEquals(format("channel %s current: %d, i: %d.  transaction action j=%d",
									channel.getName(), current, i, j), excpectedStatus,
									transactionActionInfo.getResponseStatus());
